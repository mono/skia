...
| `ffi/color.rs::SdkColorTransform::render_identity_sdr_srgb8_u16` | `source/dng_render.cpp::dng_render_task::ProcessArea`, `source/dng_reference.cpp::RefCopyArea16_R32/RefBaselineABCtoRGB/RefBaselineRGBTone/RefBaselineRGBtoRGB/RefCopyAreaR32_8`, `source/dng_1d_table.h::dng_1d_table::Interpolate` | Checked u16 input scaling, camera-white/ProPhoto clipping, intermediate RGB tone, matrix conversion and byte quantization with caller-supplied tables | `color::tests::interpolates_source_controlled_camera_profile_independently` checks selected pixels; `tools/raw_dng_color_compare.py` matches independent Rust Stage 3 and **608,400/608,400** test-only Rust Stage-4 channels against separate SDK processes | Adobe 2006–2023 notice beside method; unmodified license/technology files in `licenses/`. Test-only, not selected by public SkCodec. |
...
real-DNG zero-difference result is an intermediate Stage-4 comparison,
**not** a passing public `SkCodec` request: original default-tone and
Auto-black rendering, scale/metadata/error parity, and public selection
remain unimplemented. The earlier 1,104 one-byte differences resulted
