...
#if SK_VULKAN
// As of Chrome m152, Ganesh's GrVkGpu::Make no longer creates a default VMA-backed
// allocator when the caller supplies none; it returns null instead. Preserve the
// historical SkiaSharp behavior by building the default allocator here (mirroring the
// Graphite Vulkan shim) so existing callers that pass no allocator keep working.
static bool gr_vk_ensure_memory_allocator(skgpu::VulkanBackendContext* ctx) {
...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        if (!gr_vk_ensure_memory_allocator(&ctx)) {
            return nullptr;
        })
