...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        // As of m152, Ganesh's GrVkGpu::Make no longer auto-creates a memory allocator
        // when the caller omits one; MakeVulkan now returns null instead. Preserve the
        // historical SkiaSharp behavior by building the default VMA-backed allocator here
