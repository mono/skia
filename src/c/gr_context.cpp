...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        // Ganesh no longer auto-creates a memory allocator when one is not supplied
        // (the internal SK_USE_VMA fallback was removed upstream in m152), so build
        // the default VMA-backed allocator here to preserve existing C API behavior.
