...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        // Upstream m152 removed GrVkGpu::Make's internal fallback that created a
        // VMA-backed allocator when the caller supplied none. Restore that behaviour
        // here so existing callers that never provided an allocator keep working.
