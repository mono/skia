...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        // Skia's Ganesh Vulkan backend no longer auto-creates a memory allocator when the
        // caller supplies none (it previously did so inside GrVkGpu::Make). Preserve the
        // historical behavior so existing callers passing a null allocator keep working.
