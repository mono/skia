...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        // Ganesh's GrVkGpu::Make no longer auto-creates a memory allocator (removed in
        // Skia m152); the caller must supply one. Mirror the Graphite C shim and build the
