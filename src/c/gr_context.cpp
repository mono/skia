...
gr_direct_context_t* gr_direct_context_make_vulkan(const gr_vk_backendcontext_t vkBackendContext) {
    SK_ONLY_VULKAN(
        ...
        // Since m152, Ganesh's GrVkGpu::Make no longer auto-creates a memory
        // allocator when the caller supplies none (upstream removed the internal
        // SK_USE_VMA fallback). Build the default VMA-backed allocator here so the
        // existing C API contract (allocator optional) is preserved. Mirrors the
        // Graphite path in sk_graphite_vulkan.cpp.
