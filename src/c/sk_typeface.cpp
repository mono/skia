...
// Forward-declare the embedded font manager factory and resource header.
// The header struct is defined in SkFontMgr_custom_embedded.cpp (no public header).
// SK_EMBEDDED_FONTS is generated by tools/embed_resources.py at build time
// (e.g. NotoMono-Regular.ttf on WASM).
...
#elif defined(SK_FONTMGR_FREETYPE_EMBEDDED_AVAILABLE)
    // WASM (and other embedded platforms) use the embedded font manager with
    // fonts compiled into the binary (e.g. NotoMono-Regular.ttf). This provides
    // a fallback font for text rendering without requiring explicit font registration,
    // and also supports creating additional typefaces from data/streams via FreeType.
